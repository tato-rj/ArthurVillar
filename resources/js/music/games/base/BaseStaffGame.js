...
export class BaseStaffGame {
  ...
  start() {
    ...
    this.$staffEl.off(`staff:userNoteAdded._gate.${this.ns}`).one(`staff:userNoteAdded._gate.${this.ns}`, () => {
      $("#instructions").remove();
  ...
  _showHintNote() {
    ...
    if ($note.length) {
      $note
        .off("animationend.hint webkitAnimationEnd.hint")
        .one("animationend.hint webkitAnimationEnd.hint", () => this.staff.removeNote("hint"));
    }
  }
