...
import { stepToLetterOctave } from "../../staff/staffUtils.js";
...
export class NoteNest extends BaseStaffGame {
  ...
  _setPromptForTarget(target) {
    if (!target) return;
    const shortName = this._displayNameForLetter(target.letter);
  ...
  _isUserAnswerCorrect() {
    ...
    const [note] = notes;
    if (note.accidentalClass) return false;
    const noteState = stepToLetterOctave(this.staff, note.step);
    return String(noteState?.letter || "") === String(target.letter || "");
  }
