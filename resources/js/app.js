...
$(document).on('change', '[name$="-leaderboard-range"]', function() {
...
$(document).on('change', 'select[name="item-filter"]', function() {
...
$(document).on('click', '.subdomain-select-toggle', function(e) {
    e.stopPropagation();

    let $select = $(this).closest('.subdomains-select');
    let isOpen = $select.toggleClass('is-open').hasClass('is-open');

    $(this).attr('aria-expanded', isOpen);
});

$(document).on('click', function() {
    $('.subdomains-select.is-open')
        .removeClass('is-open')
        .find('.subdomain-select-toggle')
...
$(document).ready(function() {
    const initializePhoneInputs = function(scope) {
        ...
        root.querySelectorAll('[data-mask="phone"]').forEach(function(input) {
            ...
            const wrapper = input.closest('.form-control-phone');
            const countryCode = wrapper ? wrapper.querySelector('[data-phone-country-code]') : null;
            ...
            const applyMask = function() {
                ...
                if (!countryCode || countryCode.value === '+1') {
                    new Inputmask({"mask": "(999) 999-9999"}).mask(input);
                    return;
                }
