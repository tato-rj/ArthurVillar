...
const updateScheduleBoundaryBedStates = function(calendar) {
    const now = new Date();
    const currentMinutes = (now.getHours() * 60)
        + now.getMinutes()
        + (now.getSeconds() / 60);
    const todayIsVisible = getVisibleScheduleDates().some(function(date) {
        return toDateString(date) === todayString();
    });
    const startBedIsActive = todayIsVisible
        && currentMinutes > 0
        && currentMinutes < getTimeMinutes(scheduleStart);
    const endBedIsActive = todayIsVisible
        && currentMinutes > getTimeMinutes(scheduleEnd)
        && currentMinutes < (24 * 60);

    calendar.querySelectorAll('[data-calendar-schedule-bed]').forEach(function(marker) {
        const isStartBed = marker.dataset.calendarScheduleBed === 'start';
        const activeClass = isStartBed ? 'text-primary' : 'text-yellow';
        const isActive = isStartBed ? startBedIsActive : endBedIsActive;
...
const calendarValuesMatchSearch = function(values) {
...
const holidayMatchesCalendarSearch = function(holiday) {
...
const teachingBreakMatchesCalendarSearch = function(teachingBreak) {
    const locations = Array.isArray(teachingBreak.locations) ? teachingBreak.locations : [];
...
const recitalMatchesCalendarSearch = function(recital) {
    const students = Array.isArray(recital.students) ? recital.students : [];
    const location = recital.location || {};
...
const generalEventMatchesCalendarSearch = function(event) {
    ...
    return calendarValuesMatchSearch([
        ...
        event.external_provider === 'google' ? 'Google Calendar' : '',
    ]);
};
...
const syncGroupClassSelection = function(form) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const miniCalendars = Array.from(document.querySelectorAll('[data-mini-calendar]'));
    const miniCalendarOffcanvas = document.getElementById('calendar-mini-calendar-offcanvas');
    ...
    if (state.view === 'week'
        && !opensInStandaloneWebApp
        && isValidDate(urlState.windowStart)) {
    ...
    const closeMiniCalendarOffcanvas = function() {
        ...
        const close = miniCalendarOffcanvas.querySelector('[data-bs-dismiss="offcanvas"]');
