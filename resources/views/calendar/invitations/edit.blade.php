...
<style>
    ...
    .invitation-duration-option:hover,
    .invitation-duration-option.active {
        border-color: #0d6efd;
        background: #0d6efd;
        color: #fff;
    }

    .invitation-custom-duration {
        display: flex;
        align-items: center;
        gap: .5rem;
        max-width: 260px;
    ...
    .invitation-calendar-toolbar {
        display: flex;
        align-items: center;
        gap: .65rem;
    }

    .invitation-calendar-nav {
        position: relative;
        width: 42px;
        height: 42px;
    ...
    .invitation-calendar-label {
        margin: 0 0 0 .6rem;
        font-size: 1rem;
        font-weight: 700;
    }

    .invitation-calendar-shell {
        position: relative;
    ...
    .invitation-offscreen-indicator {
        position: absolute;
        width: 42px;
        height: 42px;
        padding: 0;
    ...
    .invitation-offscreen-indicator .invitation-overflow-count {
        top: -11px;
        right: -12px;
    }

    #invitation-calendar {
        --lm-main-color: #07883a;
        --lm-main-color-deep: #07883a;
        --lm-border-outline: #0d6efd;
    }

    #invitation-calendar .lm-schedule {
        max-height: 640px;
        min-height: 520px;
    }

    #invitation-calendar .lm-schedule thead td {
        padding-top: 14px;
        padding-bottom: 14px;
    }

    #invitation-calendar .lm-schedule thead td[data-weekday],
    #invitation-calendar .lm-schedule tbody td[data-date] {
        border-left: 1px solid #dfe3e8;
    }

    #invitation-calendar .lm-schedule tbody tr.lm-schedule-hour > td {
        border-top: 1px solid #d8dde3;
    }

    #invitation-calendar .lm-schedule tbody tr:nth-child(4n + 3) > td {
        border-top: 1px dashed #e1e5ea;
    }

    #invitation-calendar .lm-schedule tbody td.invitation-past-slot {
        background: #eceeef;
        cursor: not-allowed;
    }

    #invitation-calendar .lm-schedule-pointer {
        background-color: #07883a;
    }

    #invitation-calendar .lm-schedule-pointer:before {
        background-color: #07883a;
    }

    #invitation-calendar .lm-schedule-item {
        --lm-schedule-background: #07883a;
        border: 2px solid #fff;
        border-radius: 5px;
    ...
    #invitation-calendar .lm-schedule-item:active {
        cursor: grabbing;
    }

    #invitation-calendar .lm-schedule-item:before {
        display: none;
    }

    #invitation-calendar .lm-schedule-item:after {
    ...
    #invitation-calendar .invitation-option-delete {
        position: absolute;
        top: 4px;
        right: 5px;
        width: 23px;
    ...
    #invitation-calendar .invitation-option-delete:hover {
        background: rgba(0, 0, 0, .42);
    }

    .invitation-calendar-help {
        color: #6c757d;
        font-size: .92rem;
    }

    .invitation-calendar-error {
        display: none;
    }

    .invitation-calendar-error.is-visible {
        display: block;
    }
...
<section class="container py-5">
    ...
    <form id="invitation-form" method="POST" action="{{ $invitation ? route('calendar.invitations.update', $invitation) : route('calendar.invitations.store') }}">
        ...
        <div class="row mb-5">
            <div class="col-lg-8 col-md-10 col-12 mx-auto">
                <label class="form-label fw-bold mb-2">Duration</label>
        ...
        <div class="row mb-5">
            <div class="col-lg-8 col-md-10 col-12 mx-auto">
                ...
                <button type="button" id="invitation-calendar-previous" class="invitation-calendar-nav" aria-label="Previous week">
                ...
                <button type="button" id="invitation-calendar-next" class="invitation-calendar-nav" aria-label="Next week">
...
$(function() {
    ...
    const createVerticalOverflowIndicator = function(date, direction, events, left, edge) {
        ...
        button.addEventListener('click', function() {
            ...
            if (schedule) {
                schedule.scrollTo({
                    top: Math.max(0, timeToMinutes(target.start) - 60),
                    behavior: 'smooth',
                });
            }
        });
    ...
    const updateVerticalOverflowIndicators = function() {
        ...
        for (let index = 0; index < 7; index += 1) {
            ...
            const cell = calendarElement.querySelector(`.lm-schedule tbody td[data-date="${date}"]`);
    ...
    const normalizeEventDuration = function(event) {
        const start = Math.min(timeToMinutes(event.start), Math.max(0, 1440 - duration));
    ...
    calendarElement.addEventListener('click', function(event) {
        ...
        const start = Math.min(Number(cell.dataset.y) * 15, Math.max(0, 1440 - duration));
        const end = start + duration;
    ...
    const scrollCalendar = function() {
        requestAnimationFrame(function() {
            ...
            const firstVisibleEvent = calendar.getData().find(function(event) {
                const eventDate = parseLocalDateTime(`${event.date}T00:00`);

                return eventDate && eventDate >= visibleWeekStart && eventDate <= addDays(visibleWeekStart, 6);
            });
            const targetHour = firstVisibleEvent
                ? Math.max(0, Math.floor(timeToMinutes(firstVisibleEvent.start) / 60) - 1)
                : 7;

            if (schedule) {
                schedule.scrollTop = targetHour * 60;
            }
        });
    };
