...
<section class="container py-5">
    ...
    <div id="events-container" class="studio-table-container studio-table-container-lg">
        <table id="events-table" class="display studio-table">
            <thead>
                <tr>
                    <th>Name</th>
                    <th>Date</th>
...
$(function() {
    ...
    window.studioDataTableState.create('#events-table', {
        processing: false,
        serverSide: true,
        autoWidth: false,
        scrollX: true,
        ...
        ajax: @json(route('studio.tables.events')),
        columns: [
            {data: 'name', name: 'name'},
            ...
            {data: 'notes', name: 'notes', defaultContent: '', render: function(data, type) { return type === 'display' ? escapeHtml(data) : data; }},
            {
                data: 'id', name: 'actions', orderable: false, searchable: false, className: 'text-right',
                render: function(data) {
                    const editUrl = @json(route('studio.events.edit', ['event' => '__event__'])).replace('__event__', data);
                    const deleteUrl = @json(route('studio.events.destroy', ['event' => '__event__'])).replace('__event__', data);
