...
var bindScheduleHeaderDrag = function bindScheduleHeaderDrag(calendar, navigateByDays) {
  ...
  var finishDrag = function finishDrag(pointerId, inputType, commit) {
    ...
    var offset = Math.sign(rawOffset);
  ...
  var moveDrag = function moveDrag(pointerId, inputType, clientX, clientY, e) {
    ...
    e.preventDefault();
    var minimumX = drag.anchorX - drag.preview.dayWidth;
    var maximumX = drag.anchorX + drag.preview.dayWidth;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var scheduleHoldEdgeNavigationDelay = 1000;
  ...
  var moveScheduleByOneDay = function moveScheduleByOneDay(direction) {
    direction = Math.sign(direction);
    if (!direction || !scheduleGridViews.includes(state.view)) {
      return false;
    }
    var nextStart = addDays(getVisibleScheduleDates()[0], direction);
