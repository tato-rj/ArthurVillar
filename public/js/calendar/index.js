...
var bindScheduleHeaderDrag = function bindScheduleHeaderDrag(calendar, navigateByDays) {
  ...
  var finishDrag = function finishDrag(pointerId, inputType, commit) {
    ...
    var current = drag;
    var offset = commit && current.active && current.preview ? Math.round(-(current.baseX + current.deltaX - current.anchorX) / current.preview.dayWidth) : 0;
  ...
  var moveDrag = function moveDrag(pointerId, inputType, clientX, clientY, e) {
    ...
    e.preventDefault();
    var minimumX = drag.anchorX - drag.preview.maxDistance;
    var maximumX = drag.anchorX + drag.preview.maxDistance;
    var resistedDeltaX = deltaX * dragMovementRatio;
    var nextX = Math.max(minimumX, Math.min(maximumX, drag.baseX + resistedDeltaX));
...
var hasTransitTransportation = function hasTransitTransportation(route) {
...
var requestTravelRouteForEvent = function requestTravelRouteForEvent(event, details) {
  ...
  var request = requestJson(requestDetails.url || window.calendarTravelRouteUrl, {
    ...
    var route = payload.route && Number(payload.route.duration_seconds || 0) > 0 && hasTransitTransportation(payload.route) ? payload.route : null;
...
var renderTravelRoute = function renderTravelRoute(section, route) {
  ...
  origin.textContent = [route.origin, route.destination].filter(Boolean).map(compactPhysicalLocation).join(' to ');
